...
class MSUNet(nn.Module):
    def __init__(self) -> None:
        ...
        # region Multi-Scale feature
        
        # region Block #1
        self.msc_up1 = nn.Upsample(
            scale_factor = 2,
            mode = 'bilinear',
            align_corners = True
        )
        ...
        # region Block #2
        self.msc_conv1 = DoubleConv(
            in_channels = 384,
            out_channels = 128
        )

        self.msc_up2 = nn.Upsample(
            scale_factor = 2,
            mode = 'bilinear',
            align_corners = True
        )
        ...
        # region Block #3
        self.msc_conv2 = DoubleConv(
            in_channels = 192,
            out_channels = 64
        )

        self.msc_up3 = nn.Upsample(
            scale_factor = 2,
            mode = 'bilinear',
            align_corners = True
        )
    ...
    def forward(self, x):
        ...
        x_mid1 = self.msc_conv1(torch.cat([x3, self.msc_up1(x4)],     dim=1))
        x_mid2 = self.msc_conv2(torch.cat([x2, self.msc_up2(x_mid1)], dim=1))
        x_mid3 = self.msc_conv3(torch.cat([x1, self.msc_up3(x_mid2)], dim=1))

        x6 = self.conv6(torch.cat([self.up1(x5), x4],     dim=1))
        x7 = self.conv7(torch.cat([self.up2(x6), x_mid1], dim=1))
        x8 = self.conv8(torch.cat([self.up3(x7), x_mid2], dim=1))
        x9 = self.conv9(torch.cat([self.up4(x8), x_mid3], dim=1))
        # endregion
